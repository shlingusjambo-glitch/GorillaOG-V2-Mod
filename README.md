...
| Feature | Details |
|---|---|
| **2023 player model** | The original gorilla mesh, fur and face textures, plus 2023 shading |
| **OG Stump and Tree Room** | Stump interior, computer terminal, game-mode board, keyboard, MOTD, rules and welcome text |
| **OG Forest / Canyon / Caves / Mountain** | Historical geometry, 2023 lighting and sky, period-correct spawn points |
| **2023 City store** | Shelves, checkout, ATM, mirror, wardrobe categories and scoreboards, as they were |
| **Period-correct rendering** | Original point-filtered atlases, 2023 lightmaps, procedural 2023 sky |

## Roadmap: more eras are coming

v1.0.0 covers **August 2023**. This project is planned as a series of historical releases, one per era.
...
**The 2023 gorilla, front and side:**
...
2. **Unzip it straight into your Gorilla Tag install folder**, so you end up with:
   ...
   > **Both parts matter.** The `.dll` alone will not work. `historical-world/` must sit
   > right next to it as shown above.
3. **Launch the game.** The 2023 world loads automatically. No menu, no setup.
...
|---|---|---|---|
| `LegacyPlayer` | Restoration | `true` | 2023 player mesh and textures on/off |
| `HistoricalDiagnosticMode` | Rendering | `4` | Material debug views (`1` to `4`; leave at `4` for normal play) |
| `PlayerDebugCube` | Verification | `false` | Magenta-cube renderer test (dev only) |
| `ZoneTour` | Verification | `false` | Auto-tour plus screenshot capture (dev only) |
...
> `Release` is the public build. `Debug` is the verification build (zone tour, proof
...
├── HistoricalCityUi.cs          # 2023 City labels and UI text
...
- Fan-made project, **not affiliated with** Another Axiom or Gorilla Tag.
  Play nice, do not use mods to harass other players, and respect each lobby's rules.
